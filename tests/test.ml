...
let comp_test input : unit =
        let prf_in : t_prf = prf_of_file input in
        let temp_dir = Filename.temp_dir "" "" in
        let _ : unit = PRF_edit.decompose_file true temp_dir input in
        ...
        |true -> ()
        |false -> IO.print_to_stderr_yellow ("comp_test FAILED on " ^ input)


let comp_test_raw input : unit =
        let prf_in : t_prf_raw = prf_raw_of_file input in
        let temp_dir = Filename.temp_dir "" "" in
        let _ : unit = PRF_edit.decompose_file_raw true temp_dir input in
        let prf_out : t_prf_raw = PRF_edit.compose_prf_raw_rec temp_dir in
        let _ : int = Sys.command (String.concat " " ["rm -r";temp_dir]) in
        match prf_in = prf_out with
        |true -> ()
        |false -> IO.print_to_stderr_yellow ("comp_test_raw FAILED on " ^ input)

let itm_bidirection_test (input : string) : unit =
	let items1 : ITM_types.t_item list = ITM_main.items_of_file input in
	...
	let items2 : ITM_types.t_item list = ITM_main.items_of_file temp_path in
