...
let rec prf_raw_of_string (trace : bool) (print_tokens : bool) (raw : string) : t_prf_raw =
        try
                let s : string = PRF_sequencer.sequence_of_string trace raw in
                let lexbuf = Lexing.from_string s in
                let parser = PRF_parser.main (lexer print_tokens) in
                parser lexbuf
        with
        |PRF_parser.Error _ -> (
                match print_tokens with
                ...
                |true -> raise (Parse_error raw)
        )
        |PRF_sequencer.Error n -> (
                match trace with
                |false -> 
        ...
        |PRF_sequencer.ERROR e -> 
		let _ : unit = IO.print_to_stderr_red ("PRF_sequencer failed: " ^ e) in
		raise (Parse_error raw)
...
let prf_of_file (path:string) =
        prf_of_prf_raw (prf_raw_of_file false false path)

let prf_of_string (s : string) =
        prf_of_prf_raw (prf_raw_of_string false false s)

let prf_of_stdin () =
        prf_of_prf_raw (prf_raw_of_stdin false false)
...
let prf_contains_pred (prf : t_prf) (p : t_pred) : bool =
	let map (fml : t_fml) : t_fml option =
		if FML_main.fml_contains_pred fml p then None else Some fml
	in
	match transform_prf_opt map prf with
	|None -> true
	|Some _ -> false
