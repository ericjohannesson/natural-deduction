...
let rec subst_equiv_terms (term1 : t_term) (term2 : t_term) (eq_term1 : t_term)
    (eq_term2 : t_term) : bool =
...
and subst_equiv_term_lists (term1 : t_term) (term2 : t_term)
    (term_list1 : t_term list) (term_list2 : t_term list) : bool =
  try List.for_all2 (subst_equiv_terms term1 term2) term_list1 term_list2
  with _ -> false

let rec subst_equiv (term1 : t_term) (term2 : t_term) (fml1 : t_fml)
    (fml2 : t_fml) : bool =
  FML_main.is_closed_term term1
  && FML_main.is_closed_term term2
  &&
  ...
  | BinopApp (binop1, fml11, fml12), BinopApp (binop2, fml21, fml22) ->
      binop1 = binop2
      && subst_equiv term1 term2 fml11 fml21
      && subst_equiv term1 term2 fml12 fml22
...
let premises_of_prf (excluded : t_fml list) (prf : t_prf) : t_fml list =
  let rec aux (prf : t_prf) : t_fml list =
    ...
    | Nullary_prf (_, fml) -> []
    | Unary_prf (prf1, _, _) -> aux prf1
    | Binary_prf (prf1, prf2, _, _) -> List.concat [ aux prf1; aux prf2 ]
...
let rec validate (options : t_options) (rule_count : int) (attempt : int)
    (dischargeable : t_fml -> int option) (acc : t_prf list) (prf : t_prf) :
    t_prf option =
  match prf with
  | Atomic_prf fml -> (
      ...
      | Some i ->
          ...
          if options.discharge then
            let _ : unit =
              if options.verbose then IO.print_to_stderr "Discharging it.\n"
              else ()
            in
  ...
  | Nullary_prf (_, fml) -> (
      ...
      | PredApp (Pred "=", [ term1; term2 ]), 0 -> (
          ...
          | false ->
              validate options rule_count (attempt + 1) dischargeable acc prf)
      | _, _ -> (
          match dischargeable fml with
  ...
  | Unary_prf (prf1, _, fml) -> (
      ...
      | BinopApp (Binop "∧", conj1, conj2), _, 0 -> (
          ...
          | true -> (
              ...
              with
              | Some valid_prf1 ->
                  Some (Unary_prf (valid_prf1, Unary_rule "∧E", fml))
              | None ->
                  validate options rule_count (attempt + 1) dischargeable acc
                    prf)
          | false ->
              validate options rule_count (attempt + 1) dischargeable acc prf)
      | fml1, BinopApp (Binop "∨", disj1, disj2), 1 -> (
          ...
          | true -> (
              ...
              with
              | Some valid_prf1 ->
                  Some (Unary_prf (valid_prf1, Unary_rule "∨I", fml))
              | None ->
                  validate options rule_count (attempt + 1) dischargeable acc
                    prf)
          | false ->
      ...
      | fml1, BinopApp (Binop "→", ant, cons), 2 -> (
          ...
          | true -> (
              ...
              in
              match
                validate options (rule_count + 1) 0 dischargeable_new
                  (prf :: acc) prf1
              with
              | Some valid_prf1 ->
                  Some
      ...
      | fml1, QuantApp (Quant "∀", var, sub_fml), 3 -> (
          match
            FML_main.fml_is_fml_with_var_replaced_by_term fml1 sub_fml var
          with
          | Some (FuncApp (Func c, [])) -> (
              ...
              | false -> (
                  match
                    validate options rule_count 0 dischargeable (prf :: acc)
                      prf1
                  with
                  | Some valid_prf1 ->
                      Some (Unary_prf (valid_prf1, Unary_rule "∀I", fml))
                  | None ->
                      validate options rule_count (attempt + 1) dischargeable
                        acc prf)
              | true ->
                  validate options rule_count (attempt + 1) dischargeable acc
                    prf)
      ...
      | QuantApp (Quant "∀", var, sub_fml1), _, 5 -> (
          match
            FML_main.fml_is_fml_with_var_replaced_by_term fml sub_fml1 var
          with
          | Some _ -> (
              match
                validate options rule_count 0 dischargeable (prf :: acc) prf1
      ...
      | BinopApp (Binop "↔", eqv1, eqv2), _, 6 -> (
          match
            fml = BinopApp (Binop "→", eqv1, eqv2)
            || fml = BinopApp (Binop "→", eqv2, eqv1)
          with
          | true -> (
              match
                validate options rule_count 0 dischargeable (prf :: acc) prf1
  ...
  | Binary_prf (prf1, prf2, _, fml) -> (
      ...
      | fml1, fml2, BinopApp (Binop "∧", conj1, conj2), 0 -> (
          match
            (conj1 = fml1 && conj2 = fml2) || (conj1 = fml2 && conj2 = fml1)
          with
          | true -> (
              match
                ( validate options rule_count 0 dischargeable (prf :: acc) prf1,
              ...
              | Some valid_prf1, Some valid_prf2 ->
                  Some
                    (Binary_prf (valid_prf1, valid_prf2, Binary_rule "∧I", fml))
              | _, _ ->
                  validate options rule_count (attempt + 1) dischargeable acc
                    prf)
          | false ->
      ...
      | BinopApp (Binop "→", ant1, cons1), fml2, _, 1 -> (
          ...
          | true -> (
              ...
              | Some valid_prf1, Some valid_prf2 ->
                  Some
                    (Binary_prf (valid_prf1, valid_prf2, Binary_rule "→E", fml))
              | _, _ ->
                  validate options rule_count (attempt + 1) dischargeable acc
                    prf)
          | false ->
      ...
      | fml1, fml2, UnopApp (Unop "¬", ass), 3 -> (
          match
            fml1 = UnopApp (Unop "¬", fml2) || fml2 = UnopApp (Unop "¬", fml1)
          with
          | true -> (
              let dischargeable_new (f : t_fml) : int option =
      ...
      | fml1, fml2, _, 4 -> (
          ...
          | true -> (
              ...
              | Classical -> (
                  let dischargeable_new (f : t_fml) : int option =
                    if f = UnopApp (Unop "¬", fml) then Some rule_count
                    else dischargeable f
                  in
                  match
                    ( validate options (rule_count + 1) 0 dischargeable_new
                        (prf :: acc) prf1,
              ...
              | Intuitionistic -> (
                  ...
                  | Some valid_prf1, Some valid_prf2 ->
                      Some
                        (Binary_prf
                           (valid_prf1, valid_prf2, Binary_rule "EFQ", fml))
                  | _, _ ->
                      validate options rule_count (attempt + 1) dischargeable
                        acc prf)
              | Minimal ->
      ...
      | QuantApp (Quant "∃", var, sub_fml1), fml2, _, 5 -> (
          ...
          | true -> (
              let dischargeable_new (f : t_fml) : int option =
                match
                  FML_main.fml_is_fml_with_var_replaced_by_term f sub_fml1 var
                with
                | Some (FuncApp (Func c, [])) ->
                    let const = FuncApp (Func c, []) in
                    if
                      not
                        (List.exists (occurs_in const)
                           (fml2 :: premises_of_prf [ f ] prf2))
                    then Some rule_count
                    else dischargeable f
                | _ -> dischargeable f
              in
              match
                ( validate options (rule_count + 1) 0 dischargeable (prf :: acc)
                    prf1,
                  validate options (rule_count + 1) 0 dischargeable_new
                    (prf :: acc) prf2 )
              with
      ...
      | fml1, QuantApp (Quant "∃", var, sub_fml2), _, 6 -> (
          ...
          | true -> (
              let dischargeable_new (f : t_fml) : int option =
                match
                  FML_main.fml_is_fml_with_var_replaced_by_term f sub_fml2 var
                with
                | Some (FuncApp (Func c, [])) ->
                    let const = FuncApp (Func c, []) in
                    if
      ...
      | fml1, PredApp (Pred "=", [ term1; term2 ]), _, 8 -> (
          ...
          | false ->
              validate options rule_count (attempt + 1) dischargeable acc prf)
      | ( BinopApp (Binop "→", ant1, cons1),
          BinopApp (Binop "→", ant2, cons2),
          BinopApp (Binop "↔", eqv1, eqv2),
          9 ) -> (
          match
            ((ant1 = eqv1 && cons1 = eqv2) && ant2 = eqv2 && cons2 = eqv1)
  ...
  | Trinary_prf (prf1, prf2, prf3, _, fml) -> (
      match
        ( conclusion_of_prf prf1,
          conclusion_of_prf prf2,
          conclusion_of_prf prf3,
          fml,
          attempt )
      with
      | BinopApp (Binop "∨", left, right), fml2, fml3, _, 0 -> (
          ...
          | true -> (
              ...
              match
                ( validate options (rule_count + 1) 0 dischargeable (prf :: acc)
                    prf1,
                  validate options (rule_count + 1) 0 dischargeable_left
                    (prf :: acc) prf2,
                  validate options (rule_count + 1) 0 dischargeable_right
                    (prf :: acc) prf3 )
              with
...
let validate_prf ?(options = default_options) (prf : t_prf) : t_prf option =
  ...
  | Some valid_prf ->
      ...
      let proves_string : string =
        String.concat " "
          [ premises_string; turnstile_of_options options; conclusion_string ]
      in
      let report_string : string =
        String.concat ""
          [
            "\n";
            "Proof is VALID";
            qualification_of_options options;
            ".";
            "\n";
            "PROVES ";
            proves_string;
      ...
      in
      let _ : unit =
        if not options.quiet then IO.print_to_stderr report_string else ()
      in
      let _ : unit =
        if not options.quiet then IO.print_to_stdout proof_string else ()
      in
      Some valid_prf
  | None ->
...
let expand_and_validate_file ?(options = default_options) (path : string) : unit
  ...
  in
  IO.print_to_stdout (String.concat "\n\n" (List.map map exp_items))
