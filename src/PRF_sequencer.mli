...
val lexer_of_matrix : ?print_trace : bool -> ?automaton:t_automaton -> string array array -> string list

val lexer_of_string : ?print_trace : bool -> string -> string list

val lexer_of_file : ?print_trace : bool -> string -> string list
...
then [prf_seq_of_file prf_tree] evaluates to [Prf_seq prf_seq],

where [prf_seq] is a string conforming to the grammar also specified below.

Otherwise evaluates to [prf_seq_of_file ~print_trace:true path].

[prf_seq_of_file ~print_trace:true path] evaluates to the same thing, but prints the trace of {!val:default_automaton} to stderr, and raises exception [Automaton_error state] if [prf_tree] does not conform to the grammar.
