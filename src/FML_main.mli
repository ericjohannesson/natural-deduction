...
(** [fml_of_string fml] evaluates to an object of type [FML_types.t_fml] if
    [fml] conforms to the grammar specified in
    {{:specs/grammar/fml.txt}fml.txt}, and raises exception [Parse_error fml]
    otherwise. *)

val fml_list_of_file : string -> FML_types.t_fml list
(** [fml_list_of_file path] applies [fml_of_string] to each line of the file
    located at [path]. *)
...
val fml_is_fml_with_var_replaced_by_term :
  ...
  FML_types.t_fml ->
  FML_types.t_var ->
  FML_types.t_term option
(** [fml_is_fml_with_var_replaced_by_term fml1 fml2 var] evaluates to
    [Some term], if [term] is the first element of [closed_terms_of_fml fml1]
    such that [fml1 = subst_in_fml var term fml2] evaluates to [true], and to
    [None] if no such element exists. *)

val subst_in_fml :
  FML_types.t_var -> FML_types.t_term -> FML_types.t_fml -> FML_types.t_fml
(** [subst_in_fml var term fml] evaluates to the result of replacing every free
    occurrence of [var] in [fml] with [term], regardless of whether [term]
    contains variables that become bound. *)
