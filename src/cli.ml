...
let direction_list_of_string_list (string_list : string list) : PRF_edit.t_direction list =
	List.map direction_of_string string_list
...
let execute_arg_list (arg_list : string list) : unit = 
        ...
        |_::tl -> (
                ...
                |"validate"::option_list_path -> (
                        ...
                        |path::option_list -> (
                                let options : Main.t_options =
                                        options_of_string_list Main.default_options option_list
                                in
                                match path with
                                |"-" -> let _ = Main.validate_stdin ~options:options () in ()
                                |_ -> let _ = Main.validate_file ~options:options path in ()
                        )
                ...
                |"show"::direction_list_path -> (
                        ...
                        |path::rev_direction_list -> (
                                let directions : PRF_edit.t_direction list =
                                        List.rev (direction_list_of_string_list rev_direction_list)
                                in
                                match path with
                ...
                |"decompose"::recopt_dir_file -> (
                        ...
                        |"-R"::dir_file -> (
                                match dir_file with
                                |dir::[file] -> PRF_edit.decompose_file true dir file 
                                |_ -> raise (Invalid_argument (String.concat " " dir_file))
                        )
                        |dir_file -> (
                                match dir_file with
                                |dir::[file] -> PRF_edit.decompose_file false dir file 
                                |_ -> raise (Invalid_argument (String.concat " " dir_file))
                        )
                )
                |"decompose-raw"::recopt_dir_file -> (
                        ...
                        |"-R"::dir_file -> (
                                match dir_file with
                                |dir::[file] -> PRF_edit.decompose_file_raw true dir file 
                                |_ -> raise (Invalid_argument (String.concat " " dir_file))
                        )
                        |dir_file -> (
                                match dir_file with
                                |dir::[file] -> PRF_edit.decompose_file_raw false dir file 
                                |_ -> raise (Invalid_argument (String.concat " " dir_file))
                        )
                )
                |"compose"::recopt_dir -> (
                        match recopt_dir with
                        |"-R"::[dir] -> let _ = PRF_edit.compose_dir true dir in ()
                        |[dir] -> let _ = PRF_edit.compose_dir false dir in ()
                        |_ -> raise (Invalid_argument (String.concat " " recopt_dir))
                )
                |"compose-raw"::recopt_dir -> (
                        match recopt_dir with
                        |"-R"::[dir] -> let _ = PRF_edit.compose_dir_raw true dir in ()
                        |[dir] -> let _ = PRF_edit.compose_dir_raw false dir in ()
                        |_ -> raise (Invalid_argument (String.concat " " recopt_dir))
                )
