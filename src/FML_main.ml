...
let string_of_token (t : FML_parser.token) : string =
  ...
  | FML_parser.PREFIX_FUNC s ->
      String.concat "" [ "PREFIX_FUNC"; " "; "\'"; s; "\'" ]
  | FML_parser.INFIX_FUNC s ->
      String.concat "" [ "INFIX_FUNC"; " "; "\'"; s; "\'" ]
  | FML_parser.POSTFIX_FUNC s ->
      String.concat "" [ "POSTFIX_FUNC"; " "; "\'"; s; "\'" ]
  | FML_parser.PREFIX_PRED s ->
      String.concat "" [ "PREFIX_PRED"; " "; "\'"; s; "\'" ]
  | FML_parser.INFIX_PRED s ->
      String.concat "" [ "INFIX_PRED"; " "; "\'"; s; "\'" ]
  | FML_parser.NEG_INFIX_PRED s ->
      String.concat "" [ "NEG_INFIX_PRED"; " "; "\'"; s; "\'" ]
...
let rec fml_of_string ?(print_tokens = false) (s : string) : t_fml =
  ...
  try FML_parser.main (lexer print_tokens) b
  with _ -> (
    match print_tokens with
    | false ->
        let _ : unit =
          IO.print_to_stderr_red
            "FML_parser failed; read the the following tokens:"
        in
...
let rec string_of_fml_rec (embedded : bool) (fml : t_fml) : string =
  ...
  | PredApp (p, term_list) -> (
      ...
      | true, [ a; b ] ->
          String.concat ""
            [ string_of_term a; " "; string_of_pred p; " "; string_of_term b ]
      | false, [] -> string_of_pred p
      | _, _ ->
          String.concat "" [ string_of_pred p; string_of_term_list term_list ])
  | BinopApp (binop, fml1, fml2) -> (
      ...
      | true ->
          String.concat ""
            [
              "(";
              string_of_fml_rec true fml1;
              " ";
              string_of_binop binop;
              " ";
              string_of_fml_rec true fml2;
              ")";
            ]
      | false ->
          String.concat ""
  ...
  | UnopApp (unop, fml) ->
      String.concat "" [ string_of_unop unop; string_of_fml_rec true fml ]
  | QuantApp (quant, var, fml) ->
      String.concat ""
        [
          string_of_quant quant;
          string_of_var var;
          " ";
          string_of_fml_rec true fml;
        ]

and string_of_binop (binop : t_binop) : string = match binop with Binop s -> s
and string_of_unop (unop : t_unop) : string = match unop with Unop s -> s
and string_of_quant (quant : t_quant) : string = match quant with Quant s -> s
and string_of_pred (p : t_pred) : string = match p with Pred s -> s
and string_of_var (var : t_var) : string = match var with Var v -> v

and string_of_term_list (term_list : t_term list) : string =
  String.concat ""
    [ "("; String.concat "," (List.map string_of_term term_list); ")" ]
      ...
      | true, [ a; b ] -> (
          ...
          | true ->
              String.concat ""
                [
                  "(";
                  string_of_term_rec true a;
                  " ";
                  f;
                  " ";
                  string_of_term_rec true b;
                  ")";
                ]
          | false ->
              String.concat ""
...
and string_of_fml (fml : t_fml) : string = string_of_fml_rec false fml
and string_of_term (term : t_term) : string = string_of_term_rec false term
...
let rec closed_terms_of_fml (fml : t_fml) : t_term list =
  ...
  | BinopApp (_, fml1, fml2) ->
      List.concat [ closed_terms_of_fml fml1; closed_terms_of_fml fml2 ]
...
let rec subst_in_term (var : t_var) (replacement : t_term) (term : t_term) :
    t_term =
  match term with
  | Atom var1 -> ( match var = var1 with true -> replacement | false -> term)
  | FuncApp (func, term_list) ->
      FuncApp (func, List.map (subst_in_term var replacement) term_list)

let rec subst_in_fml (var : t_var) (term : t_term) (fml : t_fml) : t_fml =
  match fml with
  | PredApp (pred, term_list) ->
      PredApp (pred, List.map (subst_in_term var term) term_list)
  | BinopApp (binop, fml1, fml2) ->
      BinopApp (binop, subst_in_fml var term fml1, subst_in_fml var term fml2)
...
let rec vars_of_terms (term_list : t_term list) : t_var list =
  let rec aux (lst : t_term list) (acc : t_var list) : t_var list =
    ...
    | hd :: tl -> (
        ...
        | FuncApp (_, terms) ->
            aux tl (List.concat [ acc; vars_of_terms terms ]))
  in
  List.rev (aux term_list [])

let fml_is_fml_with_var_replaced_by_term (inst : t_fml) (fml : t_fml)
    (var : t_var) : t_term option =
...
let subtract (lst1 : 'a list) (lst2 : 'a list) : 'a list =
  let rec aux (lst : 'a list) (acc : 'a list) : 'a list =
    match lst with
    | [] -> acc
    | hd :: tl -> if List.mem hd lst2 then aux tl acc else aux tl (hd :: acc)
  in
  aux lst1 []
...
let free_vars_of_fml (fml : t_fml) : t_var list =
  let rec aux (bound_vars : t_var list) (f : t_fml) : t_var list =
    ...
    | UnopApp (_, fml1) -> aux bound_vars fml1
    | BinopApp (_, fml1, fml2) ->
        List.concat [ aux bound_vars fml1; aux bound_vars fml2 ]
    | QuantApp (_, (var : t_var), fml1) -> aux (var :: bound_vars) fml1
  in
  aux [] fml
