...
type t_automaton = {
	transition : (t_state -> t_symbol -> t_stack -> t_action * t_state * t_return option * t_stack);
	end_state : t_state
}
...
let default_transition (state : t_state) (symbol : t_symbol) (stack: t_stack) : t_action * t_state * (t_return option) * t_stack =
        match state with
        |State 0 -> (
                match symbol with
...
let trace (state : t_state) (row : int) (col : int) (symbol : t_symbol) (return_opt : t_return option) (stack: t_stack): unit =
        IO.print_to_stderr (String.concat " " [
		string_of_state state;
		"row"; string_of_int row;
		"col"; string_of_int col;
		string_of_symbol symbol;
		"->"; string_of_return_opt return_opt;
...
let rec lexer_of_matrix ?(print_trace = false) ?(automaton = default_automaton) (matrix : string array array) : string list =
        let rec aux (acc : string list) (state : t_state) (row : int) (col : int) (stack : t_stack) =
                ...
                |action, next_state, return_opt, next_stack -> 
                        let _ : unit = if print_trace then trace state row col symbol return_opt next_stack else () in 
                        ...
                        |Some return -> aux ((string_of_return return)::acc) next_state next_row next_col next_stack
        in
        match last_non_empty_row_of_matrix matrix with
        |None -> raise (Error "empty matrix")
        |Some i -> List.rev (aux [] (State 0) i 0 (Stack []))

let lexer_of_string ?(print_trace = false) (s : string) : string list =
...
let lexer_of_file ?(print_trace = false) (path : string) : string list =
...
let prf_seq_of_string ?(print_trace = false) (s : string) : t_prf_seq =
        Prf_seq (String.concat "" (lexer_of_string ~print_trace:print_trace s))

let prf_seq_of_file ?(print_trace = false) (path : string) : t_prf_seq =
        Prf_seq (String.concat "" (lexer_of_file ~print_trace:print_trace path))
