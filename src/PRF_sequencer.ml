...
let matrix_of_string (s : string) : string array array =
  ...
  let utf8_rows : string list list =
    List.map UTF8_segmenter.utf_8_grapheme_clusters rows
  in
...
type t_automaton = {
  transition :
    ...
    t_symbol ->
    t_stack ->
    t_action * t_state * t_token option * t_stack;
  end_state : t_state;
}
...
let push_to_stack (n : int) (stack : t_stack) : t_stack =
  match stack with Stack int_list -> Stack (n :: int_list)

let default_transition (state : t_state) (symbol : t_symbol) (stack : t_stack) :
    t_action * t_state * t_token option * t_stack =
  match state with
  | State 0 -> (
      match symbol with
  ...
  | State 3 -> (
      (* find begining of row to the right *)
      match symbol with
      | Space ->
          (Right, State 0, None, stack (* beginning of row has been found *))
      | Letter _ ->
          (Stay, State 0, None, stack (* beginning of row has been found *))
  ...
  | State 10 -> (
      ...
      | Dash ->
          ( Stay,
            State 20,
            None,
            push_to_stack 0 stack (* find beginning of roof to the left *) )
      | Out -> (Down, State 30, None, stack (* no roof! *)))
  | State 20 -> (
      (* find beginning of roof to the left *)
      match symbol with
      | Space ->
          ( Right,
            State 21,
            None,
            stack (* beginning of roof has been found; any premises on it? *) )
      | Letter _ -> raise (Automaton_error state)
      | Dash -> (Left, State 20, None, stack (* continue *))
      | Out -> (Right, State 21, None, stack))
  | State 21 -> (
      (* any premises on this roof? *)
      match symbol with
      | Space ->
          (Left, State 50, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* no premises *)
      | Letter _ ->
          (Stay, State 61, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* rule begins *)
  ...
  | State 22 -> (
      ...
      | Dash -> raise (Automaton_error state)
      | Out ->
          (Down, State 42, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* find end of floor to the right *))
  | State 23 -> (
      match symbol with
  ...
  | State 31 -> (
      ...
      | Space ->
          (Left, State 50, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* find end of floor to the left *)
      | Letter _ ->
          (Stay, State 62, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* find beginning of rule to the left *)
  ...
  | State 32 -> (
      ...
      match symbol with
      | Space ->
          (Left, State 60, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* end of floor has been found *)
      | Letter _ ->
          (Stay, State 61, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* rule begins *)
  ...
  | State 33 -> (
      ...
      | Letter _ ->
          (Stay, State 0, Some HSEP, inc_stack_hd stack (* more premises! *))
      | Dash -> raise (Automaton_error state)
      | Out ->
          ( Left,
            State 34,
            None,
            stack (* no more premises, go left to last letter*) ))
  | State 34 -> (
      match symbol with
  ...
  | State 35 -> (
      ...
      | Letter _ ->
          ...
          (* rule ends *)
      | Dash ->
          (Stay, State 60, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* floor has no rule *)
  ...
  | State 41 -> (
      ...
      | Letter _ -> raise (Automaton_error state)
      | Dash ->
          (Right, State 42, Some (RSEP (stack_hd stack)), stack_tl stack)
          (* find end of floor to the right *)
  ...
  | State 50 -> (
      (* find end of of floor to the left*)
      match symbol with
      ...
      | Out -> (Stay, State 100, None, stack (* no floor! *)))
  | State 60 -> (
      (* end of floor-dashes have been found *)
      match symbol with
      | Space -> raise (Automaton_error state)
      | Letter _ -> raise (Automaton_error state)
  ...
  | State 62 -> (
      ...
      | Out -> raise (Automaton_error state))
  | State 70 -> (
      (* find end of row to the left *)
      match symbol with
      | Space ->
          ( Stay,
            State 80,
            None,
            stack (* end of row has been found, reading empty *) )
      | Letter _ ->
          ( Stay,
            State 90,
            None,
            stack
            (* end of row has been found, reading the last letter of a word *)
          )
      | Dash -> raise (Automaton_error state)
  ...
  | State 80 -> (
      ...
      | Out -> raise (Automaton_error state))
  | State 90 -> (
      (* find beginning of formula to the left *)
      match symbol with
  ...
  | State 91 -> (
      ...
      | Space ->
          ( Right,
            State 92,
            None,
            stack (* find beginning of formula to the right *) )
      | Letter _ -> (Left, State 90, None, stack)
      | Dash -> (Left, State 90, None, stack)
      | Out ->
          ( Right,
            ...
            stack (* find beginning of formula to the right *) ))
  | State 92 -> (
      (* find beginning of formula to the right *)
      match symbol with
...
let default_automaton : t_automaton =
  { transition = default_transition; end_state = State 100 }

let symbol_of_matrix (row : int) (col : int) (matrix : string array array) :
    t_symbol =
  try match matrix.(row).(col) with " " -> Space | "-" -> Dash | s -> Letter s
  with _ -> Out

let string_of_state (state : t_state) : string =
  match state with State n -> "State " ^ string_of_int n
...
let trace (state : t_state) (row : int) (col : int) (symbol : t_symbol)
    (token_opt : t_token option) (stack : t_stack) : unit =
  IO.print_to_stderr
    (String.concat " "
       [
...
let last_non_empty_row_of_matrix (matrix : string array array) : int option =
  let rec aux (i : int) : int option =
    try match matrix.(i) with [||] -> aux (i - 1) | _ -> Some i
    with _ -> None
  in
  aux (Array.length matrix - 1)

(* Lexer *)

let rec lexer_of_matrix ?(print_trace = false) ?(automaton = default_automaton)
    (matrix : string array array) : t_token list =
  let rec aux (acc : t_token list) (state : t_state) (row : int) (col : int)
      (stack : t_stack) =
    if state = automaton.end_state then acc
    else
      ...
      | action, next_state, token_opt, next_stack -> (
          let _ : unit =
            if print_trace then trace state row col symbol token_opt next_stack
            else ()
          in
          ...
          | Some token ->
              aux (token :: acc) next_state next_row next_col next_stack)
  in
...
let lexer_of_string ?(print_trace = false) (s : string) : t_token list =
  ...
  try lexer_of_matrix ~print_trace matrix
  with Error "empty matrix" -> raise (Error "empty string")

let lexer_of_file ?(print_trace = false) (path : string) : t_token list =
  try lexer_of_string ~print_trace (IO.string_of_file path)
  with Error "empty string" -> raise (Error "empty file")

let prf_seq_of_string ?(print_trace = false) (s : string) : t_prf_seq =
  Prf_seq
    (String.concat ""
       (List.map string_of_token (lexer_of_string ~print_trace s)))

let prf_seq_of_file ?(print_trace = false) (path : string) : t_prf_seq =
  Prf_seq
    (String.concat ""
       (List.map string_of_token (lexer_of_file ~print_trace path)))
