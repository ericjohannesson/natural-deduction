...
let fml_precedence_test input output: unit =
        match Main.fml_list_of_file input = output with
        |true -> ()
        |false -> IO.print_to_stderr ("fml_precedence_test FAILED on " ^ input)
...
let _ : unit = fml_fixpoint_test fml4_in
...
let _ : unit = fml_precedence_test fml4_in fml4_out
